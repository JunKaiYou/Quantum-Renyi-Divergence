...
function output = func_val(sigma , alpha, p, A) 
    output = 0;
    temp_power = (1-alpha)/(2*alpha);
    sigma_t = sigma ^ temp_power;
    for x = 1:length(p)
        temp = A(:,:,x);
        output = output + p(x) * log(trace((sigma_t * temp * sigma_t) ^ alpha));
    end
...
function output = grad(sigma , alpha, p, A) 
    output = 0 ;
    temp_power = (1-alpha)/(2*alpha);
    sigma1 = sigma ^ (-temp_power);
    sigma_t = sigma ^ temp_power;

    for x = 1:length(p)
        temp = A(:,:,x);

        total_matrix = (sigma_t * temp * sigma_t) ^ alpha;
        [~,Frechet] = powerm_pade_fre(sigma, (1-alpha)/alpha, total_matrix);
        
        output = output + p(x) * (sigma1 * Frechet * sigma1) ./ (trace(total_matrix));
        
    end
    output = output * (alpha/(alpha-1));
end
